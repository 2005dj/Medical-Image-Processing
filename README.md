...
### Requirements
```bash
pip install numpy scipy scikit-image imageio pandas matplotlib seaborn
```

### Data
Download the [OSIC Pulmonary Fibrosis Progression](https://www.kaggle.com/competitions/osic-pulmonary-fibrosis-progression) dataset and place `train.zip` and the metadata CSV in the project directory.

### Run
Open the notebook and run the cells top to bottom:
```bash
jupyter notebook Pulmonary_Fibrosis_Image_Analysis.ipynb
```
...
For each patient the pipeline produces:
- a binary lung mask,
- a computed lung volume (cm³),
- a raw CT / mask / segmentation-overlay visualization for the slice containing the most lung tissue,

plus a scatter plot of **lung volume vs. FVC (% predicted)** across all patients with the correlation coefficient.

---

## Notes & limitations

- HU thresholds are tuned for lung tissue and may need adjusting for scans with heavy fibrosis or unusual acquisition settings.
- Segmentation is classical (thresholding + morphology), not learning-based — fast and interpretable, but it can struggle with severely diseased lungs where tissue density shifts.
- The volume-vs-FVC correlation is a validation check on a small cohort, not a diagnostic claim.

## Possible extensions
- Scale to the full patient set.
- Add a learning-based segmentation model as a comparison baseline.
- Export per-patient volumes and QC flags to a structured table for downstream analysis.
